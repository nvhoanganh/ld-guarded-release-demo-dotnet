...
The demo covers both metric types that feed Guarded Releases:

- **1 custom metric** — `checkout-latency`, emitted via `LdClient.Track()`
- **1 automatic metric** — `http.latency;route=/api/checkout`, auto-generated by LaunchDarkly's
  Observability module from OpenTelemetry trace data
...
| Metric definition | Average per user, then Average, **lower is better** |
| Unit | `ms` |
...
- Metrics to monitor: **`checkout-latency`** (✓ Auto rollback) and **`http-latency-checkout`**
...
- `POST /api/checkout` evaluates `new-checkout-flow` with a per-user `Context`, simulates
  fast/stable behaviour when `false` and slow/flaky behaviour when `true`, then emits the
  measured latency via `LdClient.Track("checkout-latency", context, null, elapsedMs)`.
- ASP.NET Core auto-instrumentation creates a server span for every `/api/checkout` request.
  ...
  `http.latency;route=/api/checkout` event you can build a metric on top of.
